...
foreach ($f in $allHtml) {
  ...
  $new = $html
  $new = [regex]::Replace($new,'href="https://www\.all-in-one-converter\.net(?!/)', 'href="https://www.all-in-one-converter.net/', 'IgnoreCase')
  $new = [regex]::Replace($new,'content="https://www\.all-in-one-converter\.net(?!/)', 'content="https://www.all-in-one-converter.net/', 'IgnoreCase')
...
foreach ($loc in $Locales) {
  ...
  $new = [regex]::Replace($new, '(?is)<link\b[^>]*\brel\s*=\s*"alternate"[^>]*\bhreflang\s*=\s*"[^"]+"[^>]*>\s*', '', 'IgnoreCase')
...
[int]$dedupCount = 0
$pattern = [regex]::new('(?is)<link\b[^>]*\brel\s*=\s*"alternate"[^>]*\bhreflang\s*=\s*"(?<lang>[^"]+)"[^>]*\bhref\s*=\s*"(?<href>[^"]+)"[^>]*>', 'IgnoreCase')
foreach ($f in $allHtml) {
  $enc,$html = Read-Text $f.FullName
...
[int]$deferCount = 0
$scriptOpen = [regex]::new('(?is)<script(?<attrs>[^>]*)>', 'IgnoreCase')
foreach ($f in $allHtml) {
  $enc,$html = Read-Text $f.FullName
