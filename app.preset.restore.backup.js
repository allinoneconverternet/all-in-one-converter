...
function createFileCard(index, file, badgeText = '', statusText = t('queued')) {
  ...
  card.innerHTML = `
    <div class="file-meta">
      ...
      <div class="sub">${prettySize(file.size)} • ${typeLabel}</div>
    </div>
...
;/* === Surgeon Patch: Localized dropdown + URL preset === */
(function(){
  if (window.__SURGEON_PATCH__) return; window.__SURGEON_PATCH__ = true;

  function t(k, v){ try { return (window.t ? window.t(k, v) : (k)); } catch { return k; } }
  function optLabelSafe(val){ try { return (window.optLabel ? window.optLabel(val) : (val.toUpperCase())); } catch { return val.toUpperCase(); } }
  ...
  try{
    ...
    var qualityWrap = document.querySelector('#quality-wrap, #qualityWrap, .quality-wrap, [data-role=""quality""]');
    if (!target) return;
    ...
    if (qualityWrap) qualityWrap.style.display = (target.value === 'jpeg' || target.value === 'webp') ? '' : 'none';
  } catch {}
};
    var wanted = alias[q] || q;
    ...
    // 2) Fallback to last slug chunk (…-to-pdf, …-v-pdf, …-から-pdf, etc.)
    if (!wanted) {
      var path = location.pathname.replace(/\/index\.html$/i,'');
      var seg  = (path.split('/').filter(Boolean).pop() || '').toLowerCase();
      var m = seg.match(/([a-z0-9.]+)$/i);
    ...
    if (wanted && target.querySelector('option[value="'+wanted+'"]')) {
      target.value = wanted;
      if (qualityWrap) qualityWrap.style.display = (wanted==='jpeg' || wanted==='webp') ? '' : 'none';
    } else {
      // Ensure quality UI is correct even if we couldn't match
      if (qualityWrap) qualityWrap.style.display = (target.value==='jpeg' || target.value==='webp') ? '' : 'none';
    }
  } catch {}
};
      var wanted = alias[cand] || cand;
      if (wanted && target.querySelector('option[value="'+wanted+'"]')) {
        target.value = wanted;
        if (qualityWrap) qualityWrap.style.display = (wanted==='jpeg' || wanted==='webp') ? '' : 'none';
      }
  ...
  window.presetTargetFromURL = presetTargetFromURL;

  function localizedRebuild(){
    try{
      var target = document.querySelector('#target-format, #targetFormat, select[name=target], select#target');
      var qualityWrap = document.querySelector('#quality-wrap, #qualityWrap, .quality-wrap, [data-role="quality"]');
      if (!target || !qualityWrap) return;

      var groupsOrder = ['text','documents','spreadsheets','images','media'];
      var groupLabelKey = { text:'group_text', documents:'group_documents', spreadsheets:'group_spreadsheets', images:'group_images', media:'group_media' };

      var allowedSet = (typeof window.possibleTargetsForFiles==='function' && window.state && Array.isArray(window.state.files))
                       ? window.possibleTargetsForFiles(window.state.files) : null;

      target.innerHTML = '';
      for (var gi=0; gi<groupsOrder.length; gi++){
        var key = groupsOrder[gi];
        if (!(window.ENABLE_OUTPUTS && window.ENABLE_OUTPUTS[key])) continue;
        var items = (window.TARGET_GROUPS && window.TARGET_GROUPS[key]) || [];
        var filtered = allowedSet ? items.filter(function(p){ return allowedSet.has(p[0]); }) : items;
        if (!filtered.length) continue;

        var og = document.createElement('optgroup');
        og.label = t(groupLabelKey[key]);
        for (var i=0; i<filtered.length; i++){
          var val = filtered[i][0];
          var o = document.createElement('option');
          o.value = val;
          o.textContent = optLabelSafe(val);
          og.appendChild(o);
        }
        target.appendChild(og);
      }

      var keep = (target.querySelector('option[value="'+target.value+'"]') ? target.value : (target.querySelector('option') && target.querySelector('option').value) || '');
      if (keep) target.value = keep;

      presetTargetFromURL(); // override defaults based on URL
      var v = target.value; qualityWrap.style.display = (v==='jpeg' || v==='webp') ? '' : 'none';
    } catch(e){ /* no-op */ }
  }

  // Wrap existing builders (if present) to also preset from URL
  function wrap(fn){ return function(){ try{ if (typeof fn==='function') fn(); } finally { presetTargetFromURL(); } }; }
  if (window.buildTargets) window.buildTargets = wrap(window.buildTargets);
  if (window.refreshTargetDropdown) window.refreshTargetDropdown = localizedRebuild;

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', localizedRebuild);
  } else {
    localizedRebuild();
  }
})();
