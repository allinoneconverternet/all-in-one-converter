...
(function initI18N() {
  ...
  window.showBanner = function (msg, kind = 'info') {
    try {
      ...
      // QUIET: swallow technical banners, but keep console logs in debug mode
      if (window.QUIET_TECH && isTechnicalMessage(m)) {
        if (window.DEBUG_CONVERTER) {
          console[kind === 'error' ? 'error' : 'log']('[tech]', m);
        }
        return; // do not render in #banner
      }

      // Call any previous renderer (compat), but don't return early
