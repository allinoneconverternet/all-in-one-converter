#!/usr/bin/env node
// scripts/generate-conversion-pages.mjs
...
async function main() {
    ...
    for (const row of chosen) {
        ...
        const relPath = slug.replace(/^\//, ''); // remove leading slash
        const dir = path.join(projectRoot, relPath);
        await fs.mkdir(dir, { recursive: true });

        const related = pickRelated(records, row, 8)
        ...
        const html = template
            .replaceAll('{{title}}', escapeHtml(row.title))
            .replaceAll('{{meta_description}}', escapeHtml(row.meta_description))
            .replaceAll('{{canonical}}', escapeHtml(row.canonical || ''))
            .replaceAll('{{h1}}', escapeHtml(row.h1))
